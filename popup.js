...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function createAnswerCard(data = {}) {
        ...
        div.innerHTML = `
            <div class="answer-header">
                <span class="answer-title">答案 ${index}</span>
                <button class="btn-del">×</button>
            </div>
            <div class="row" style="margin-bottom:8px;">
                <div class="col" style="flex:0.4;">
                    <label>分值</label>
                    <input type="number" class="ans-score" value="${data.score || 2}">
                </div>
                <div class="col">
                    <label>关键词 (逗号分隔)</label>
                    <input type="text" class="ans-keywords" value="${data.keywords || ''}" placeholder="例如: 毅力,坚持">
                </div>
            </div>
            <label>参考内容</label>
            <textarea class="ans-content" rows="2" placeholder="输入标准答案内容...">${data.content || ''}</textarea>
        `;
    ...
    function updateIndexes() {
        ...
        cards.forEach((card, idx) => {
            card.querySelector('.answer-title').innerText = `答案 ${idx + 1}`;
        });
    }
    ...
    saveBtn.addEventListener('click', () => {
        ...
        chrome.storage.local.set({ gradingConfig: config }, () => {
